...
void main() {
  ...
  // Create a 3-stripe pattern for different visualizations
  float stripePattern = mod(floor(gl_FragCoord.x / 50.0), 3.0);
  
  if (stripePattern > 1.5) {
    // Right third: Show ray directions
    fragColor.rgb = mix(fragColor.rgb, avgRayColor, 0.7);
  } else if (stripePattern > 0.5) {
    // Middle third: Show distance field visualization
    ...
    vec3 distFieldColor = vec3(distFieldValue * 1.0); // Amplify for visibility
    fragColor.rgb = mix(fragColor.rgb, distFieldColor, 0.8);
  } else {
    // Left third: Show probe grid
