...
vec4 radiance_interval(vec2 uv, vec2 dir, float a, float b) {
  ...
  // Debug: Return visualization based on maximum distance sampled (shows SDF influence)
  // Brighter colors = larger distances in SDF, darker = closer to surfaces
  float distVisualization = maxDistSampled * 20.0; // Amplify for visibility
  return vec4(vec3(distVisualization), 1.0);
}
...
void main() {
  ...
  // Debug: Visualize depth intervals for current cascade
  // Color-code based on the distance range this cascade covers
  float depthNorm = (p.intervalStart + p.intervalEnd) * 0.5;
  vec3 depthColor = vec3(
    depthNorm,           // R increases with distance
    1.0 - depthNorm,     // G decreases with distance
    0.2                  // B constant for visibility
  );
  
  // Debug: Track merging events
  float mergeEvents = 0.0;
  float totalRays = 0.0;
  ...
  // Calculate merge ratio (percentage of rays that merged from lower cascade)
  float mergeRatio = (totalRays > 0.0) ? mergeEvents / totalRays : 0.0;
  
  // Create a 5-stripe pattern for different visualizations
  float stripePattern = mod(floor(gl_FragCoord.x / 50.0), 5.0);
  
  if (stripePattern > 3.5) {
    // Fifth stripe: Show depth interval visualization
    fragColor.rgb = mix(fragColor.rgb, depthColor, 0.7);
  } else if (stripePattern > 2.5) {
    // Fourth stripe: Show cascade merge visualization
    // Magenta/purple indicates high merge ratio, cyan indicates low merge
    vec3 mergeColor = mix(vec3(0.0, 1.0, 1.0), vec3(1.0, 0.0, 1.0), mergeRatio);
    fragColor.rgb = mix(fragColor.rgb, mergeColor, 0.8);
  } else if (stripePattern > 1.5) {
    // Third stripe: Show ray directions
    fragColor.rgb = mix(fragColor.rgb, avgRayColor, 0.7);
  } else if (stripePattern > 0.5) {
    // Second stripe: Show distance field visualization
    // Sample the distance field at current pixel to show SDF values
    float distFieldValue = texture(uDistanceField, gl_FragCoord.xy / uResolution).r;
    vec3 distFieldColor = vec3(distFieldValue * 15.0); // Amplify for visibility
    fragColor.rgb = mix(fragColor.rgb, distFieldColor, 0.8);
  } else {
    // First stripe: Show probe grid
