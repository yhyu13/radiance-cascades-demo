...
- `10_zero_init_cascade_textures_step11_followup_plan_review.md` - critique of the Step 11 follow-up zero-init cascade textures plan against current `src/demo3d.cpp`, `src/demo3d.h`, `res/shaders/radiance_3d.comp`, and `gl_helpers.h`.
- `11_gi_pass_1080p_perf_analysis_plan_review.md` - critique of the 1080p perf analysis plan against current `src/main3d.cpp`, `src/demo3d.cpp`, and `src/demo3d.h`. Found wrong line ref for `rdocForceRebuildCount`, `SetWindowSize` after-init wrong pattern, 2.25× raymarch estimate caveat, 6 vs 10 dispatch sites.
- `12_gi_pass_scaling_experiment_plan_review.md` - critique of the scaling experiment plan against current `src/demo3d.h`, `src/demo3d.cpp`, `res/shaders/`. Found `volumeResolution` runtime not compile-time, no public setters exist, cascade re-allocation overhead, `giBlurRadius` default mismatch, `meshSDFReady` invalidation chain debate.
- `13_perf_tooling_step12_impl_review.md` - critique of the Step 12 perf tooling implementation note against current `src/main3d.cpp`, `src/demo3d.h`, `src/demo3d.cpp`, `doc/5/claude_plan/gi_pass_1080p_perf_analysis_plan.md`, `doc/5/claude_plan/gi_pass_scaling_experiment_plan.md`, `doc/5/claude_plan/perf/gi_pass_1080p_perf_analysis.md`, and `doc/5/claude_plan/perf/gi_pass_scaling_experiment.md`. Found wrong FOV-fit line ref (`:4993` vs actual `:5080-5098`), internal contradiction ("no instrumentation needed" vs "±2-5× variance"), ImGui vs CLI invalidation inconsistency (3-line vs 6-line for same trigger), "what's next" should recommend tooling before optimizations given own evidence, "no new GPU resources" imprecise, sscanf vs atoi inconsistency, Config B anomaly direction-flipping severity understated.

Targets reviewed (updated):

- `doc/5/claude_plan/perf_tooling_step12_impl.md`

16. The Step 12 perf tooling implementation is a genuine landing — all 4 CLI flags present and functional, parse-before-init pattern correct, 3 scaling setters with proper invalidation chains, 22 RenderDoc captures landed. The `setCascadeC0Res` setter correctly omits `meshSDFReady = false` (codex 12 F8 partial reject, lighting-only change). However: the FOV-fit math line reference is wrong (`:4993` vs actual `:5080-5098`, propagated from the 1080p perf analysis plan), the document contradicts itself by claiming "no per-pass instrumentation needed" while admitting RenderDoc single-shot has ±2-5× variance that defeated Exp 3+4, the ImGui handler for `cascadeC0Res` does 3-line invalidation while the CLI setter does 6-line (same trigger, different depth — consistency gap), the "what's next" section should recommend tooling/variance control before optimization measurements given the document's own evidence, "no new GPU resources" is imprecise (runtime reallocation happens via setter), and the Config B anomaly's direction-flipping severity (720p 4× slower than 1080p) is understated as just "magnitude noise."
