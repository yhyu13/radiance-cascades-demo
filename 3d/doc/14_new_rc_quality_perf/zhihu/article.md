...
7. **根错了，叶子再漂亮也会裂。** 拿截图当正确、拿系数当质量、拿另一套算法的耗时当基线——下一轮仍会在同一处裂开。

**结论**

给实时全局光照做验收，不是把它调到「看起来像路径追踪」，而是先有一本对得上的账：实现有没有跑偏、近似细到哪一档、空活长什么样。能抄走的是这套切割，不是某个仓库里的文件名。

**PS.** 只带走一句的话：正确不是观感。图好看了，不等于还在跑原来那套算法。

**PPS.** 这和「把图调得更像路径追踪」不是一回事。路径追踪是第二问的报告，不是第一问的通行证。

**PPPS.** 盒子、档位、线程数都会过时。如果你在自己的系统里拿截图当正确、拿增益当质量、拿另一套算法的毫秒数当基线——换场景时，仍会在同一处裂开。那才是这篇要拦的事。
