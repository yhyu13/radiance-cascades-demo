...
**A2 implementation (done 2026-08-17):**
...
- `raymarch.frag` — added `uniform float uSolidAngleLUT[256]`; `sampleProbeDir` and
  `sampleProbeDirDetail` now integrate `Σ L cos⁺ ΔΩ α` and fold `1/π` (Lambert) into
  `irrad`, replacing the renormalized mean `Σ(L·cos·α)/Σ(cos·α)`. α-gate preserved until A5.
- `demo3d.cpp` — uploads the LUT matching `uAtlasDirRes` (D8/D16) on the raymarch program.
- Rebuilt + smoke-tested (no GLSL errors; render stable). Post-A2 capture:
  `tools/captures/rdoc_frame_frame418.rdc` (+ `_thumb.png`).

**A2 remaining:** runtime constant-atlas integration D=8/16, zero/partial-visibility
cases, and the pinned CV1 A/B (cascade vs PT-reference) — the measurement gate before A3.
