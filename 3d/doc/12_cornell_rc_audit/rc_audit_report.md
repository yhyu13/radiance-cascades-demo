...
| **A2** Analytic integration + solid-angle tests | ✅ ANALYTIC PASS | Per-bin octahedral ΔΩ integrated numerically; `ΣΔΩ≈4π`, hemisphere `≈2π`, `∫cos⁺dΩ=π` all verified (§7.1). Shader edit pending. |
...
**A2 next action (shader edit):** replace `sampleProbeDir`'s renormalized mean
`Σ(L·cos·α)/Σ(cos·α)` with `irrad = Σ_b L_b cos⁺_b ΔΩ_b` (α-gating preserved until A5),
with `ΔΩ_b` from a D²-entry LUT (or the equal-area octahedral map), then rebuild +
constant-atlas integration D=8/16 + pinned CV1 A/B.
