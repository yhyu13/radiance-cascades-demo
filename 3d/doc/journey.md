...
| *(A2–A9 execution, measured)* | Executed A2 (octahedral ΔΩ consumer) and A5 step 1 (dropped the consumer α-gate that was discarding surface-hit radiance). CV1 (cascade vs PT, cornell/cam0): **0.43 → 0.66**. Single-bounce isolation (MB off) = **0.22** → point-light geometry is the dominant remaining gap (Stage 11c's "87%"). |
| *(correction: directional light)* | The PT reference is **not** point-light-only — `pt_reference.comp` has `uLightDir`/`uUseDirectionalLight` (`:52-55,260-262`, wired in `demo3d.cpp:3594-3599`). My "A4 blocked / PT goes black" was a **light-direction sign error**: I passed the ShaderToy `GetSunDirection` value (`-0.6755,1,0.7374`, y=+1) directly, but the cascade/PT convention is `lightDirection` = light-travel direction, so the sun ended up below the floor. Flipped to `0.6755,-1,-0.7374`: pt_direct 0.00013 → 0.018. |
