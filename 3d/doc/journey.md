...
| *(A2–A9 execution, measured)* | Implemented A2 (octahedral ΔΩ consumer). Its CV1 gate **did NOT pass** (ratio_mean 0.43; on the locked gate p95(\|ln\|)≈13.8, dim%≈88% — 28×/18× over the 0.50/5% bar). Removed the consumer α-gate and read the brighter 0.66 as progress — **wrong**: the α-gate is leak-prevention (transmittance), so 0.66 is known-invalid (bleed conflated). Reverted. |
| *(correction: attribution)* | MB-off 0.22 is the known Cornell collapse (Stage 8, "diagnostic not fix") → **multi-bounce H-A' (Stage 11d)**, not point-light geometry (11c superseded). The PT reference is **not** point-light-only (`uLightDir`/`uUseDirectionalLight` present); "black" was a sun-direction sign error. A4 stays circular until directional PT self-validates. |
