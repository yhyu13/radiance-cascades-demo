"""v2.4.b A/B analyzer — Default (K=0) vs IndirectClamp K=2 at cornell/cam0/N=2048.
...
pre-committed v2.4.b gate from doc/7/v24b_indirect_clamp_impl.md.

Pre-committed verdict bands (locked before sweep):
  STRONG:    |p95| drops >=30%, ratio shift <=0.10, bright% drops >=3pp, dim% not worse by >3pp
  MARGINAL:  |p95| drops 10-30%, ratio shift <=0.15, bright% drops 1-3pp, dim% not worse by >5pp
...
VARIANT_DIR = 'tools/v24b_clamp/captures'
STEM_DEF    = f'cv1_cornell_cam0_mbon_g100_hyb0_N{N:04d}_m17_postfix'
STEM_VAR    = f'v24b_cornell_cam0_mbon_g100_hyb0_N{N:04d}_m17_clampK{K}'
OUT_JSON    = 'tools/v24b_clamp/v24b_results.json'
...
print(f'===== v2.4.b A/B analysis - Default (K=0) vs IndirectClamp (K={K}) =====')
...
m_def  = compute_metrics(casc_d, ptf_d, ptd_d, 'Default_K0')
...
m_var  = compute_metrics(casc_v, ptf_v, ptd_v, f'Clamp_K{K}')
...
print()
print(f"  Delta ratio       : {delta['d_ratio']:+.3f}")
print(f"  Delta |p50|       : {delta['d_p50']:+.3f}")
print(f"  Delta |p95|       : {delta['d_p95']:+.3f}  ({delta['p95_pct_change']:+.1f}%)")
print(f"  Delta dim%        : {delta['d_dim_pct']:+.2f} pp")
print(f"  Delta bright%     : {delta['d_bright_pct']:+.2f} pp")

p95_drop_pct = -delta['p95_pct_change']
ratio_shift  = abs(delta['d_ratio'])
bright_drop  = -delta['d_bright_pct']
dim_change   = delta['d_dim_pct']

print()
print('=== Pre-committed gate (locked in doc/7/v24b_indirect_clamp_impl.md) ===')
print(f'  |p95| drop:    {p95_drop_pct:+.1f}%   (STRONG >=30%, MARGINAL 10-30%, DEAD <10%)')
print(f'  ratio shift:   {ratio_shift:.3f}     (STRONG <=0.10, MARGINAL <=0.15, DEAD >0.15)')
...
if   (p95_drop_pct >= 30 and ratio_shift <= 0.10 and bright_drop >= 3 and dim_change <= 3):
    verdict = 'STRONG'
    action  = 'Ship as opt-in preset IndirectClamp_K2; document shadow tradeoff. NEVER ship as Default.'
elif (p95_drop_pct >= 10 and ratio_shift <= 0.15 and bright_drop >= 1 and dim_change <= 5):
    verdict = 'MARGINAL'
    action  = 'Document; opt-in preset only if user requests. Pivot to v2.5 architectural.'
else:
    verdict = 'DEAD'
    action  = 'Revert (CLI flag stays for debug only); pivot to v2.5 architectural.'
