...
$outDir = "tools/v3_m1_shader_contrib/captures_$Scene"
$tag = "m1shadercontrib_${Scene}_baseline_N$('{0:D4}' -f $N)_m17"
...
Write-Host "[m1-shader-contrib] $tag"
if ($DryRun) {
...
& $exe @argList |
    Select-String -Pattern "measurement-camera|use-hybrid|useDirectionalGI|hdr-exr|probe-stats|screenshot saved" |
