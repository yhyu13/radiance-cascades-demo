...
$rel="tools/10_refactor/phase5_merge/runs/$id/reference_merge_report.json";$path=Join-Path $root $rel
if($DryRun){Write-Host "$exe --runtime-shell=app3d --validate-reference-merge --reference-merge-report=$rel";exit 0}
...
Push-Location $root;try{& $exe --runtime-shell=app3d --validate-reference-merge --reference-merge-report=$rel;$code=$LASTEXITCODE}finally{Pop-Location}
if(-not(Test-Path $path)){throw "missing report"};$r=Get-Content -Raw $path|ConvertFrom-Json
Write-Host "[phase5] report=$path result=$($r.result)";if($code-ne 0-or$r.result-ne"PASS"){exit 1}
